...
<?php
  ...
  <?php
	foreach ($artistas as $artista) {
		...
		$insert = $db65-> prepare("INSERT INTO usuarios (nombre_usuario, contrasena, tipo, nombre) values(?, ?, 'artista', ?);");
		...
		$insert -> execute(array($nombre_usuario_artista,strval(random_int(100000, 999999)), $artista[0]));		
	}

	foreach ($productoras as $productora) {
        
		$insert_2 = $db65-> prepare("INSERT INTO usuarios (nombre_usuario, contrasena, tipo, nombre, pais) values(?, ?, 'productora', ?, ?);");
		$nombre_usuario_prod = '';
		...
		$insert_2 -> execute(array($nombre_usuario_productora,strval(random_int(100000, 999999)), $productora[1], $productora[2]));
	}
  ?>
